 ...
 # <-------------------------------- Video Downloading Functions ------------------------------->
    ...
    def download_video(self,semaphore,thread_id) -> None:
        ...
        try:
            yt = YouTube(self.url)
            self.title = yt.title
            self.folder_name = '_'.join(self.title.split()[0:2])
            stream = yt.streams.get_lowest_resolution() 
            print(f"Downloading video titled: {self.title}")
            self.video_path = stream.download(output_path=self.folder_name, filename=self.folder_name+'.mp4')
...
# <-------------------------------- Video Analysis Functions ------------------------------->
    ...
    def extract_audio(self,thread_id) -> None:
        """
        ...
        """
        try:
            self.audio_path = os.path.join(self.folder_name, self.folder_name + ".wav")
    ...
    def transcribe_audio(self,thread_id) -> None:
        """
        ...
        try:
            ...
            self.text_path = os.path.join(self.folder_name, self.folder_name + ".txt")
    ...
    def sentiment_analysis(self,thread_id) -> None:
        """
    ...
    def translate_text(self, thread_id, lang_from, lang_to, lang_to_name) -> None:
        """
        ...
        try:
            ...
            self.translated_text_path = os.path.join(self.folder_name, self.folder_name + "_"+lang_to_name+".txt")
    ...
    # def extract_emotions(self) -> None:
    #     print(f"SUBTASK 5 :: Extracting emotions from the video {self.title}")

    #     text_to_analyse = self.get_text_from_file()
    #     doc = nlp(text_to_analyse)
    #     full_text = ' '.join([sent.text for sent in doc.sents])
    #     emotion = NRCLex(full_text)
    #     print("SUBTASK 5 :: Detected Emotions and Frequencies:")
